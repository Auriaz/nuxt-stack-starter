...
const weekDays = computed(() => {
  ...
  return props.showWeekends ? days : days.filter(item => ![0, 6].includes(item.getDay()))
})
...
function createEventForTime(day: Date, hour: number) {
  emit('timeStart', { date: new Date(day.getFullYear(), day.getMonth(), day.getDate(), hour, 0, 0), shiftKey: false })
  emit('timeEnd')
}

function getTimeMenuItems(day: Date, hour: number) {
  return [
    [
      {
        label: 'Nowe wydarzenie',
        icon: 'i-lucide-plus',
        onClick: () => createEventForTime(day, hour)
      }
    ],
    [
      {
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-lg font-semibold">
          {{ weekDays[0]?.toLocaleDateString('pl-PL') }} - {{ weekDays[weekDays.length - 1]?.toLocaleDateString('pl-PL') }}
        </p>
      </div>
      <UButton
        variant="outline"
        ...
        >
          ...
          >
            <div
              ...
              data-day-column
              :style="{ left: `${(index / weekDays.length) * 100}%`, width: `${100 / weekDays.length}%` }"
            >
              <UContextMenu
                v-for="hour in hours"
                :key="hour"
                :items="getTimeMenuItems(day, hour)"
              >
                <div
