/**
 * Rozszerzenie TipTap: ghost text (sugestia AI) + skróty Tab/Escape/Mod-j.
 * Wzorowane na dokumentacji Nuxt UI Editor — With AI completion.
 */
...
export interface CompletionOptions {
  /** Opóźnienie debounce (ms) przed wywołaniem completion. */
  debounce?: number
  /** Czy automatycznie wywoływać completion przy pisaniu. */
  autoTrigger?: boolean
  /** Znaki, po których nie wywoływać completion (np. /, :, @). */
  triggerCharacters?: string[]
  /** Wywołane, gdy ma być wywołane completion. */
  onTrigger?: (editor: Editor) => void
  /** Wywołane, gdy użytkownik zaakceptuje sugestię (Tab). */
  onAccept?: () => void
  /** Wywołane, gdy użytkownik odrzuci sugestię (Escape). */
  onDismiss?: () => void
}
...
export const completionPluginKey = new PluginKey('ai-completion')
