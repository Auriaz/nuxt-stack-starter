...
import { removeFriendUseCase } from '~~/domain/friends/removeFriend.usecase'
import { FriendRequestConflictError, FriendRequestNotFoundError } from '~~/domain/friends/errors'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const request = await removeFriendUseCase(userId, targetId, friendsRepository)
    const removedUser = request.senderId === userId ? request.receiver : request.sender
    const actorName = request.senderId === userId
      ? (request.sender.name ?? request.sender.username)
      : (request.receiver.name ?? request.receiver.username)
    await createNotification({
      userId: removedUser.id,
      type: 'info',
      title: 'Usunieto ze znajomych',
      message: `${actorName} usunal Cie ze znajomych.`,
