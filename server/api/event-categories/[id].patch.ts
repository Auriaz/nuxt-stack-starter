...
import { UpdateEventCategoryInputSchema } from '#shared/schemas/event-category'
import { createEventCategoryRepository } from '~~/server/repositories/eventCategory.repo'
import { updateCategory } from '~~/domain/event-category/use-cases/updateCategory'
import { EventCategoryNotFoundError, EventCategoryPermissionError, EventCategoryConflictError } from '~~/domain/event-category/errors'
 ...
 *
 * Aktualizacja kategorii zdarzeń
 */
export default defineEventHandler(async (event) => {
  ...
  const parseResult = safeParse(UpdateEventCategoryInputSchema, body)
  ...
  // 6. Use-case with error handling
  try {
    ...
    if (err instanceof EventCategoryConflictError) {
      throw createError({ status: 409, statusText: 'Conflict', data: { error: { code: 'CONFLICT', message: err.message } } })
    }
    throw err
  }
})
