import { safeParse } from 'valibot'
import { CreateEventCategoryInputSchema } from '#shared/schemas/event-category'
import { createEventCategoryRepository } from '~~/server/repositories/eventCategory.repo'
import { createCategory } from '~~/domain/event-category/use-cases/createCategory'
import { EventCategoryPermissionError, EventCategoryConflictError } from '~~/domain/event-category/errors'
import { PERMISSIONS } from '~~/shared/permissions'
import { hasPermission, hasRole } from '~~/domain/access/access.service'

/**
 * POST /api/event-categories
 *
 * Tworzenie nowej kategorii zdarzeń
 */
export default defineEventHandler(async (event) => {
  // 1. Auth
  const session = await requireUserSession(event)
  const user = session.user as { id?: number, role?: string, permissions?: string[] } | undefined
  const userId = user?.id

  if (!userId) {
    throw createError({
      status: 401,
      statusText: 'Unauthorized',
      data: { error: { code: 'UNAUTHORIZED', message: 'Missing user id' } }
    })
  }

  // 2. Parse & validate body
  const body = await readBody(event)
  const parseResult = safeParse(CreateEventCategoryInputSchema, body)

  if (!parseResult.success) {
    throw createError({
      status: 400,
      statusText: 'Bad Request',
      data: {
        error: {
          code: 'VALIDATION_ERROR',
          message: 'Nieprawidłowe dane wejściowe',
          issues: parseResult.issues.map(issue => ({
            path: issue.path?.map(p => String(p.key)).join('.') ?? 'unknown',
            message: issue.message
          }))
        }
      }
    })
  }

  // 3. Permissions
  if (!hasRole(user, 'admin')
    && !hasPermission(user, PERMISSIONS.ADMIN_ACCESS)
    && !hasPermission(user, PERMISSIONS.CATEGORY_SYSTEM_MANAGE)) {
    throw createError({
      status: 403,
      statusText: 'Forbidden',
      data: { error: { code: 'FORBIDDEN', message: 'Insufficient permissions' } }
    })
  }
  const permissions = user.permissions ?? []

  // 4. Repository
  const repo = createEventCategoryRepository()

  // 5. Use-case with error handling
  try {
    const category = await createCategory(
      {
        data: parseResult.output,
        userId,
        permissions
      },
      repo
    )

    // 6. Response
    return {
      data: category,
      meta: {
        requestId: event.context.requestId
      }
    }
  } catch (err) {
    if (err instanceof EventCategoryPermissionError) {
      throw createError({ status: 403, statusText: 'Forbidden', data: { error: { code: 'FORBIDDEN', message: err.message } } })
    }
    if (err instanceof EventCategoryConflictError) {
      throw createError({ status: 409, statusText: 'Conflict', data: { error: { code: 'CONFLICT', message: err.message } } })
    }
    throw err
  }
})
