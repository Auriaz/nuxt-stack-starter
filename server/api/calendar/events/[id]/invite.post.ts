...
import { InviteParticipantsInputSchema } from '#shared/schemas/calendar'
import type { InviteParticipantsInput } from '#shared/types/calendar'
import { inviteParticipantsUseCase } from '~~/domain/calendar/inviteParticipants.usecase'
...
export default defineEventHandler(async (event) => {
  ...
  const parseResult = safeParse(InviteParticipantsInputSchema, body)
  ...
  try {
    const data = await inviteParticipantsUseCase(
      { userId, permissions: user?.permissions ?? [], eventId, input: parseResult.output as InviteParticipantsInput },
