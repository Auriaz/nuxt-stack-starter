...
import { getTopPagesUseCase } from '~~/domain/analytics/getTopPages.usecase'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await getTopPagesUseCase(parsed.output, provider)
  if (isErr(result)) {
    throw createError({
