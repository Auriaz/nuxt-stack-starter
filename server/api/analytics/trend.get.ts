 ...
 * Trend dzienny (wykres): sesje i pageviews per dzień.
...
import { getTrendUseCase } from '~~/domain/analytics/getTrend.usecase'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await getTrendUseCase(parsed.output, provider)
  if (isErr(result)) {
    throw createError({
